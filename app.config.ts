...
function loadEnv(variant: AppVariant): void {
    ...
    if (fs.existsSync(envPath)) {
        ...
        console.log(`✅ Loaded ${loadedCount} environment variables from .env.${variant}`);
    } else {
        console.warn(`⚠️  Environment file not found: ${envPath}`);
        console.log('💡 Creating default environment...');

        // Create default .env file
        const defaultEnv = variant === 'dev'
...
const expoConfig: ExpoConfig = {
    ...
    ios: {
        ...
        infoPlist: {
            NSFaceIDUsageDescription: 'Use Face ID to securely access your Oly App account',
        },
    },
    android: {
        adaptiveIcon: {
